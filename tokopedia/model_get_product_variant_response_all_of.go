/*
Tokopedia

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 1.0
Contact: dev@sirclo.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package tokopedia

import (
	"encoding/json"
)

// checks if the GetProductVariantResponseAllOf type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &GetProductVariantResponseAllOf{}

// GetProductVariantResponseAllOf struct for GetProductVariantResponseAllOf
type GetProductVariantResponseAllOf struct {
	Data *Variant `json:"data,omitempty"`
}

// NewGetProductVariantResponseAllOf instantiates a new GetProductVariantResponseAllOf object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewGetProductVariantResponseAllOf() *GetProductVariantResponseAllOf {
	this := GetProductVariantResponseAllOf{}
	return &this
}

// NewGetProductVariantResponseAllOfWithDefaults instantiates a new GetProductVariantResponseAllOf object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewGetProductVariantResponseAllOfWithDefaults() *GetProductVariantResponseAllOf {
	this := GetProductVariantResponseAllOf{}
	return &this
}

// GetData returns the Data field value if set, zero value otherwise.
func (o *GetProductVariantResponseAllOf) GetData() Variant {
	if o == nil || IsNil(o.Data) {
		var ret Variant
		return ret
	}
	return *o.Data
}

// GetDataOk returns a tuple with the Data field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetProductVariantResponseAllOf) GetDataOk() (*Variant, bool) {
	if o == nil || IsNil(o.Data) {
		return nil, false
	}
	return o.Data, true
}

// HasData returns a boolean if a field has been set.
func (o *GetProductVariantResponseAllOf) HasData() bool {
	if o != nil && !IsNil(o.Data) {
		return true
	}

	return false
}

// SetData gets a reference to the given Variant and assigns it to the Data field.
func (o *GetProductVariantResponseAllOf) SetData(v Variant) {
	o.Data = &v
}

func (o GetProductVariantResponseAllOf) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o GetProductVariantResponseAllOf) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Data) {
		toSerialize["data"] = o.Data
	}
	return toSerialize, nil
}

type NullableGetProductVariantResponseAllOf struct {
	value *GetProductVariantResponseAllOf
	isSet bool
}

func (v NullableGetProductVariantResponseAllOf) Get() *GetProductVariantResponseAllOf {
	return v.value
}

func (v *NullableGetProductVariantResponseAllOf) Set(val *GetProductVariantResponseAllOf) {
	v.value = val
	v.isSet = true
}

func (v NullableGetProductVariantResponseAllOf) IsSet() bool {
	return v.isSet
}

func (v *NullableGetProductVariantResponseAllOf) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableGetProductVariantResponseAllOf(val *GetProductVariantResponseAllOf) *NullableGetProductVariantResponseAllOf {
	return &NullableGetProductVariantResponseAllOf{value: val, isSet: true}
}

func (v NullableGetProductVariantResponseAllOf) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableGetProductVariantResponseAllOf) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


