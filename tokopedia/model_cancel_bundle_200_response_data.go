/*
Tokopedia API

Tokopedia API

API version: 1.0
Contact: dev@sirclo.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package tokopedia

import (
	"encoding/json"
)

// checks if the CancelBundle200ResponseData type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &CancelBundle200ResponseData{}

// CancelBundle200ResponseData struct for CancelBundle200ResponseData
type CancelBundle200ResponseData struct {
	BundleId *int64 `json:"bundle_id,omitempty"`
}

// NewCancelBundle200ResponseData instantiates a new CancelBundle200ResponseData object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCancelBundle200ResponseData() *CancelBundle200ResponseData {
	this := CancelBundle200ResponseData{}
	return &this
}

// NewCancelBundle200ResponseDataWithDefaults instantiates a new CancelBundle200ResponseData object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCancelBundle200ResponseDataWithDefaults() *CancelBundle200ResponseData {
	this := CancelBundle200ResponseData{}
	return &this
}

// GetBundleId returns the BundleId field value if set, zero value otherwise.
func (o *CancelBundle200ResponseData) GetBundleId() int64 {
	if o == nil || IsNil(o.BundleId) {
		var ret int64
		return ret
	}
	return *o.BundleId
}

// GetBundleIdOk returns a tuple with the BundleId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CancelBundle200ResponseData) GetBundleIdOk() (*int64, bool) {
	if o == nil || IsNil(o.BundleId) {
		return nil, false
	}
	return o.BundleId, true
}

// HasBundleId returns a boolean if a field has been set.
func (o *CancelBundle200ResponseData) HasBundleId() bool {
	if o != nil && !IsNil(o.BundleId) {
		return true
	}

	return false
}

// SetBundleId gets a reference to the given int64 and assigns it to the BundleId field.
func (o *CancelBundle200ResponseData) SetBundleId(v int64) {
	o.BundleId = &v
}

func (o CancelBundle200ResponseData) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o CancelBundle200ResponseData) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.BundleId) {
		toSerialize["bundle_id"] = o.BundleId
	}
	return toSerialize, nil
}

type NullableCancelBundle200ResponseData struct {
	value *CancelBundle200ResponseData
	isSet bool
}

func (v NullableCancelBundle200ResponseData) Get() *CancelBundle200ResponseData {
	return v.value
}

func (v *NullableCancelBundle200ResponseData) Set(val *CancelBundle200ResponseData) {
	v.value = val
	v.isSet = true
}

func (v NullableCancelBundle200ResponseData) IsSet() bool {
	return v.isSet
}

func (v *NullableCancelBundle200ResponseData) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCancelBundle200ResponseData(val *CancelBundle200ResponseData) *NullableCancelBundle200ResponseData {
	return &NullableCancelBundle200ResponseData{value: val, isSet: true}
}

func (v NullableCancelBundle200ResponseData) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCancelBundle200ResponseData) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


